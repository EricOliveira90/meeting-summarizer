...
- `FileManagerService.deleteJobFiles()` reconstructs the upload path from the original filename, while creation sanitizes the artifact filename.
- `pnpm test` reports workspace and future Vite config-loader warnings.
- Final verification: 387 tests passed; the production build and built-runtime smoke passed.
